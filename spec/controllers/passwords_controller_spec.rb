...
describe PasswordsController, type: %i[controller mailer] do
  before do
  ...
  context 'as json' do
    describe '#create' do
      ...
      it 'returns 422 with no email' do
      ...
      context "using an email address that doesn't belong to a user" do
        ...
        it 'does not send an email to the account email address' do
      ...
      context 'using an email address that belongs to a user' do
        it 'queues the email for delayed sending' do
        ...
        it 'uses devise to send the password reset email' do
        ...
        it 'sends an email from the no-reply email address' do
          post :create, params: user_email_attrs
          email = ActionMailer::Base.deliveries.first
          expect(email.from).to include('no-reply@zooniverse.org')
        end

        it 'sends an email to the account email address' do
          post :create, params: user_email_attrs
          email = ActionMailer::Base.deliveries.first
          expect(email.to).to include(user.email)
        end

        it 'contains the correct route url for the server' do
          post :create, params: user_email_attrs
          email = ActionMailer::Base.deliveries.first
    ...
    describe '#update' do
      ...
      context 'when not supplying a valid reset token' do
        ...
        it 'returns return 422 and a meaningful error response body' do
          expect(response.status).to eq(422)
      ...
      context 'when supplying a valid reset token' do
        ...
        context 'with a database user' do
          ...
          context 'when supplying a invalid new password' do
            ...
            it 'returns a meaningful error response body' do
  ...
  context 'as html' do
    ...
    describe '#update' do
      ...
      context 'when not supplying a valid reset token' do
        ...
        it 'does not have updated the password' do
          user.reload
          expect(user.valid_password?(new_password)).not_to eq(true)
        end
      end
