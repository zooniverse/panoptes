...
describe SubjectGroups::VirtualGroupSubjectSelectorSerializer do
  ...
  describe '.page' do
    it 'serializes an array of virtual subjects under subjects key' do
      result = described_class.page({}, [virtual_subject_group1], selection_context)
      expect(result[:subjects]).to be_an(Array)
      expect(result[:subjects].size).to eq(1)
      expect(result[:subjects].first[:id]).to eq(virtual_subject_group1.id.to_s)
    end
    ...
    it 'honors page_size and page parameters' do
      result = described_class.page({ page_size: 1 }, [virtual_subject_group1, virtual_subject_group2], selection_context)
      expect(result[:subjects].map { |s| s[:id] }).to eq([virtual_subject_group1.id.to_s])

      result2 = described_class.page({ page_size: 1, page: 2 }, [virtual_subject_group1, virtual_subject_group2], selection_context)
      expect(result2[:subjects].map { |s| s[:id] }).to eq([virtual_subject_group2.id.to_s])
    end
