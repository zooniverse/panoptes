...
describe UserCollectionPreferencePolicy do
  describe 'scopes' do
    let(:user) { create(:user) }

    let(:private_collection) { build :collection, private: true }

    let(:public_preference) { build(:user_collection_preference, user: user, public: true) }
    let(:private_preference) { build(:user_collection_preference, user: user) }
    let(:other_user_public_preference) { build(:user_collection_preference, public: true) }
    let(:other_user_private_preference) { build(:user_collection_preference) }
    let(:private_project_public_preference) { build(:user_collection_preference, public: true, collection: private_collection) }
    ...
    subject do
      PunditScopeTester.new(UserCollectionPreference, api_user)
    end
