...
describe Projects::CreateClassificationsExport do
  ...
  it 'should update an existing export if one exists' do
    params = create_params
    params[:media].delete(:metadata)
    export = create(:medium, linked: project, type: "project_classifications_export", content_type: content_type, metadata: {})
    operation.with(project: project).run!(create_params)
    export.reload
    expect(export.metadata).to include("recipients" => [user.id])
  end
  ...
  it 'should update the updated_at timestamp of the export' do
    params = create_params
    params[:media].delete(:metadata)
    export = create(:medium, linked: project, type: "project_classifications_export", content_type: content_type, metadata: {recipients: [user.id]}, updated_at: 5.days.ago)
    operation.with(project: project).run!(create_params)
    export.reload
