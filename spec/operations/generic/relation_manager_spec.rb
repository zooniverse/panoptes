...
describe Generic::RelationManager do
  ...
  subject(:relation_manager) { described_class.new(resource.class, ApiUser.new(user)) }

  describe "#update_relations" do
    context "many-to-many" do
      it 'should replace the relation with new items' do
        updated = relation_manager.update_relation(resource,
                                                :subjects,
      ...
      context 'empty link' do
        it 'should set to empty collection' do
          updated = relation_manager.update_relation(resource, :subjects, [])
      ...
      context "not found" do
        it 'should raise an error' do
          expect do
            relation_manager.update_relation(resource, :subjects, [-10, -2])
    ...
    context "one-to-many and polymorphic" do
      ...
      it 'should add the new relation to the resource' do
        updated = relation_manager.update_relation(*update_params)
      ...
      context "not found" do
        ...
        it 'should raise an error' do
          expect do
            relation_manager.update_relation(*update_params)
    ...
    context "belongs-to-many" do
      it 'should add the new relation to the resource' do
        updated = relation_manager.update_relation(resource, :projects, [project.id])
        expect(updated).to match_array([project])
      end

      context 'empty link' do
        it 'should set to empty collection' do
          updated = relation_manager.update_relation(resource, :projects, [])
          expect(updated).to be_empty
        end
      end
    end
  ...
  describe "#add_relation" do
    context "to-many" do
      it 'should add the new relation to the resource' do
        relation_manager.add_relation(resource, :subjects, subjects.map(&:id).map(&:to_s))
        expect(resource.subjects).to include(*subjects)
      end

      it 'should allow adding no items' do
        relation_manager.add_relation(resource, :subjects, subjects.map(&:id).map(&:to_s))
        relation_manager.add_relation(resource, :subjects, [])
        expect(resource.subjects).to include(*subjects)
      end
    end
    ...
    context "to-one" do
      it 'should replace the old relation' do
        ...
        relation_manager.add_relation(resource, :owner, params)
    ...
    context "belongs-to-many" do
      it 'should replace the old relation' do
        relation_manager.add_relation(resource, :projects, [project.id])
        expect(resource.projects).to include(*project)
      end

      it 'should allow adding no items' do
        relation_manager.add_relation(resource, :projects, [project.id])
        relation_manager.add_relation(resource, :projects, [])
        expect(resource.projects).to include(*project)
      end
    end
  end
  ...
  describe "#destroy_relation" do
    ...
    it 'should remove the linked relations' do
      relation_manager.destroy_relation(resource, :subjects, del_string)
    ...
    context "has_many" do
      ...
      it 'should destroy the unlinked items' do
        expect do
          ...
          relation_manager.destroy_relation(resource, :workflows, del_string)
